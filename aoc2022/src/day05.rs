...
#[aoc_generator(day5)]
pub fn input_generator(input: &str) -> (CrateStack, Vec<(i32, i32, i32)>) {
    let (stacks, commands) = input.split_once("\n\n").unwrap();
    (parse_stacks(stacks), parse_commands(commands))
}

pub fn parse_stacks(s: &str) -> CrateStack {
...
#[aoc(day5, part1)]
pub fn part1((stacks, commands): &(CrateStack, Vec<(i32, i32, i32)>)) -> String {
...
#[aoc(day5, part2)]
pub fn part2((stacks, commands): &(CrateStack, Vec<(i32, i32, i32)>)) -> String {
    let mut stacks = stacks.clone();
    for com in commands {
        let mut holding = Vec::<char>::new();
        for _ in 0..com.0 {
